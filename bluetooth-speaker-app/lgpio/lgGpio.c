...
static inline void xSetBit(long *b, int n)
{
   *b |= ((long)1 << n);
}

static inline void xChangeBit(long *b, int n)
{
   *b ^= ((long)1 << n);
}

static inline void xClearBit(long *b, int n)
{
   *b &= ~((long)1 << n);
}

static inline int xTestBit(long b, int n)
{
   return !!(b & ((long)1 << n));
}

static inline void xAssignBit(long *b, int n, int value)
{
...
void xGroupWrite(
   lgChipObj_p chip, int gpio, long groupBits, long groupMask)
{
   int i;
   lgLineInf_p GPIO;
   struct gpio_v2_line_values lv;
...
int lgGroupWrite(
   int handle, int gpio, long groupBits, long groupMask)
{
   int status;
   int i;
   lgLineInf_p GPIO;
