...
int group_read(int sbc, int handle, int group, long *value)
{
   int status;
   int bytes;
   lgExtent_t ext[1];
...
int group_write(
   int sbc, int handle, int group, long groupBits, long groupMask)
{
   lgExtent_t ext[2];
   long parq[] = {groupBits, groupMask};
   uint32_t pars[] = {handle&0xffff, group};
...
int lgu_get_internal(int sbc, int config_id, long *config_value)
{
   int status;
   int bytes;
   lgExtent_t ext[1];
...
int lgu_set_internal(int sbc, int config_id, long config_value)
{
   lgExtent_t ext[2];
   long parsQ[] = {config_value};
...
long lgu_timestamp(void)
{
   struct timespec xts;

   clock_gettime(CLOCK_REALTIME, &xts); // get current time

   return ((long)1E9 * xts.tv_sec) + xts.tv_nsec;
}
